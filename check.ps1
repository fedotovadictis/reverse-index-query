...
Add-SourceFeatureAssessment -Ctx $ctx -Id 'minimum.query_tree' -Level 'minimum' -Category 'algorithm' -Requirement 'JSON query tree TERM AND OR' -Patterns @('TERM|Term','AND|And','OR|Or') -Match 'all'
...
Add-SourceFeatureAssessment -Ctx $ctx -Id 'excellent.memory_estimate' -Level 'excellent' -Category 'performance' -Requirement 'Index memory estimate' -Patterns @('memory|Memory','bytes|Bytes') -Match 'all'
Add-SourceFeatureAssessment -Ctx $ctx -Id 'excellent.index_tradeoffs' -Level 'excellent' -Category 'documentation' -Requirement 'Index versus scan tradeoffs are documented' -Patterns @('worse|trade.?off','scan','index') -Match 'all'
